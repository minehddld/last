...
# 세션 상태 초기화 (현재 보고 있는 코인)
if "active_coin" not in st.session_state:
    st.session_state.active_coin = safe_defaults[0] if safe_defaults else all_options[0]
...
st.sidebar.markdown("---")
st.sidebar.subheader("🔍 코인 선택 및 이동")

# 관심종목 이동 버튼 생성
if watchlist_names:
    st.sidebar.caption("👇 빨간 버튼을 클릭하면 즉시 이동합니다.")
    cols = st.sidebar.columns(2)
    for i, coin in enumerate(watchlist_names):
        k_name = coin.split(" ")[0]
        if cols[i % 2].button(f"🔴 {k_name}", key=f"btn_{coin}", use_container_width=True, type="primary"):
...
def render_realtime_dashboard():
    ...
    # -----------------------------
    st.markdown("---")
    st.subheader("📑 실시간 계단식 호가 (업비트 UI)")
    
    orderbook = pyupbit.get_orderbook(ticker)
    
    try:
        ticker_info = requests.get(f"https://api.upbit.com/v1/ticker?markets={ticker}").json()[0]
        recent_trades = requests.get(f"https://api.upbit.com/v1/trades/ticks?market={ticker}&count=9").json()
        ...
        if orderbook:
            ...
            <style>
                ...
                .ob-wrap {{ display: flex; width: 100%; max-width: 900px; border: 1px solid #d3d6dc; background: white; margin: 0 auto; }}
                .col {{ flex: 1; display: flex; flex-direction: column; width: 33.33%; }}
                .col-center {{ flex: 1; border-left: 1px solid #f1f1f4; border-right: 1px solid #f1f1f4; }}
                .row {{ height: {row_h}px; display: flex; align-items: center; position: relative; box-sizing: border-box; }}
                ...
                .val {{ z-index: 2; padding: 0 10px; width: 100%; display: flex; align-items: center; box-sizing: border-box; }}
                .up {{ color: #c84a31; }}
                .down {{ color: #1261c4; }}
                .even {{ color: #333; }}
                .info-box {{ height: 320px; padding: 15px; font-size: 11px; display: flex; flex-direction: column; justify-content: center; border-bottom: 1px solid #f1f1f4; box-sizing: border-box; }}
                .info-row {{ display: flex; justify-content: space-between; margin-bottom: 8px; }}
                .trade-header {{ height: {row_h}px; display: flex; align-items: center; justify-content: space-between; padding: 0 10px; background-color: #f8f8f8; border-top: 1px solid #e1e1e1; border-bottom: 1px solid #e1e1e1; font-weight: bold; color: #666; font-size: 11px; box-sizing: border-box; }}
                .trade-row {{ height: {row_h}px; display: flex; align-items: center; justify-content: space-between; padding: 0 10px; border-bottom: 1px solid #f9f9f9; box-sizing: border-box; font-size: 11px; }}
            </style>
            </head>
            <body>
            <div class="ob-wrap">
            ...
            html_str += '''
                    ...
                    <div style="height: 320px; display: flex; flex-direction: column;">
                        <div class="trade-header" style="border-top: none;"><span>체결가</span><span>체결량</span></div>
            '''
            for trade in recent_trades[:9]:
                color = "up" if trade['ask_bid'] == 'BID' else "down"
                html_str += f'<div class="trade-row"><span class="{color}">{format_price(trade["trade_price"])}</span><span class="{color}">{trade["trade_volume"]:,.3f}</span></div>'
            for _ in range(9 - len(recent_trades[:9])):
                html_str += '<div class="trade-row"><span></span><span></span></div>'
            ...
            for ask in asks:
                ...
                c_class = "up" if p > prev_close else "down" if p < prev_close else "even"
                html_str += f'<div class="row ask-bg"><div class="val {c_class}" style="justify-content: center; font-weight: bold;">{format_price(p)} <span style="font-weight: normal; font-size: 10px; margin-left: 8px;">{pct:+.2f}%</span></div></div>'
            
            for bid in bids:
                p = bid['bid_price']
                pct = (p - prev_close)/prev_close * 100
                c_class = "up" if p > prev_close else "down" if p < prev_close else "even"
                html_str += f'<div class="row bid-bg"><div class="val {c_class}" style="justify-content: center; font-weight: bold;">{format_price(p)} <span style="font-weight: normal; font-size: 10px; margin-left: 8px;">{pct:+.2f}%</span></div></div>'
            ...
            '''
            components.html(html_str, height=660, scrolling=False)
...
col_scan1, col_scan2, col_empty = st.columns([1.5, 2, 6.5])
with col_scan1:
    surge_price = st.number_input("탐지 기준 상승률 (%)", value=5.0, step=1.0)
with col_scan2:
    st.markdown("<div style='margin-top: 27px;'></div>", unsafe_allow_html=True) 
    do_scan = st.button("🚀 전체 마켓 즉시 스캔", use_container_width=True)
